...
class TechnicalDocumentsRelationManager extends RelationManager
{
    protected static string $relationship = 'technicalDocuments';

    protected static ?string $recordTitleAttribute = 'file_name';

    public static function getTitle(\Illuminate\Database\Eloquent\Model $ownerRecord, string $pageClass): string
    {
        return 'الوثائق الفنية';
    }
    ...
    public static function canViewForRecord(\Illuminate\Database\Eloquent\Model $ownerRecord, string $pageClass): bool
    {
        ...
        // العضو الفني يرى الوثائق الفنية فقط
        $memberType = static::getUserMemberType();
        if ($memberType === MemberType::Technical && $ownerRecord->current_review_stage === ReviewStage::Technical) {
            return true;
        }
        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('file_name')
                    ...
                    ->searchable()
                    ->sortable()
                    ->url(fn ($record) => route('documents.technical.view', $record))
                    ->openUrlInNewTab()
                    ->color('primary')
                    ->icon('heroicon-o-eye')
                    ->iconPosition('after'),
            ...
            ->actions([
                ...
                Action::make('download')
                    ...
                    ->color('success')
                    ->url(fn ($record) => route('documents.technical.download', $record))
                    ->openUrlInNewTab(),
            ]);
    }
}
