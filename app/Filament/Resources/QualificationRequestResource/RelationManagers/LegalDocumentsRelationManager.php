...
class LegalDocumentsRelationManager extends RelationManager
{
    ...
    public static function canViewForRecord(\Illuminate\Database\Eloquent\Model $ownerRecord, string $pageClass): bool
    {
        ...
        // العضو القانوني يرى الوثائق القانونية فقط
        $memberType = static::getUserMemberType();
        if ($memberType === MemberType::Legal && $ownerRecord->current_review_stage === ReviewStage::Legal) {
            return true;
        }
        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('file_name')
                    ...
                    ->searchable()
                    ->sortable()
                    ->url(fn ($record) => route('documents.legal.view', $record))
                    ->openUrlInNewTab()
                    ->color('primary')
                    ->icon('heroicon-o-eye')
                    ->iconPosition('after'),
            ...
            ->actions([
                ...
                Action::make('download')
                    ...
                    ->color('success')
                    ->url(fn ($record) => route('documents.legal.download', $record))
                    ->openUrlInNewTab(),
            ]);
    }
}
